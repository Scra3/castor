...
function addressOf(item: Item, yamlPath: string): string {
  ...
  throw new LayoutFileError(
    `${yamlPath}: item without a usable \`id\` (and the name contains characters forbidden in a path). ` +
      'Do not remove the `id` fields generated by `layout pull`.',
  )
}
...
function indexByIdentity(items: Item[], yamlPath: string): Map<string, Item> {
  ...
  for (const item of items) {
    ...
    if (key === undefined) {
      throw new LayoutFileError(`${yamlPath}: item without \`id\` or \`name\` — unable to identify it.`)
    }

    if (index.has(key)) throw new LayoutFileError(`${yamlPath}: two items share the same identity (${key}).`)
    index.set(key, item)
  }
...
function diffScalarLike(
  ...
  if (localValue === undefined) {
    ...
    ctx.bucket.warnings.push(
      `${yamlPath}: this field cannot be removed — restore its value (or use \`layout patch\`).`,
    )
...
export function diffDomain(domain: LayoutDomain, remote: unknown, local: unknown): DiffResult {
  ...
  for (const op of ops) {
    if (!matchesWhitelist(domain, op)) {
      throw new Error(
        `Internal bug: op generated outside the whitelist (${op.op} ${op.path}). Please report this case — nothing was sent.`,
      )
    }
  }
