...
const HEADER_COMMENT = ` forest-layout.yml — generated by \`forest-onboard layout pull\`.
 Edit this file then run \`forest-onboard layout diff\` / \`layout apply\`.
...
export function parseLayoutFile(content: string): {docs: LayoutFileDoc; scope: Partial<LayoutScope>} {
  ...
  try {
    ...
    throw new LayoutFileError(`Invalid YAML: ${error instanceof Error ? error.message : String(error)}`)
  }
  ...
  if (!header) {
    throw new LayoutFileError('Missing `forest:` header. Generate the file with `forest-onboard layout pull`.')
  }
