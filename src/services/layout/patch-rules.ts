 ...
 * grained workspace component options…) remain reachable via `layout patch`.
 */
import type {LayoutDomain} from './types.js'
...
const scalar = (prop: string, extra: Partial<ScalarRule> = {}): ScalarRule => ({kind: 'scalar', prop, ...extra})
const opaque = (prop: string, extra: Partial<OpaqueRule> = {}): OpaqueRule => ({kind: 'opaque', prop, ...extra})

/** Chart properties (viewEdit and dashboards), patchable and removable per the server. */
const CHART_PROPS: Rule[] = [
  'name', 'type', 'aggregator', 'aggregateFieldName', 'filter', 'query', 'apiVersion', 'sourceCollectionId',
  'groupByFieldName', 'timeRange', 'limit', 'labelFieldName', 'relationshipFieldName', 'numeratorChartId',
  'denominatorChartId', 'objective', 'displaySettings', 'description', 'subTitle',
...
// Same id classes as the server's patch-handler.ts.
...
const COLLECTION_SCALARS = [
  'displayName', 'displayNamePlural', 'icon', 'restrictedToSegments', 'defaultSortingOrder',
  'defaultSortingFieldName', 'displayFieldName',
].join('|')

const FIELD_PROPS = [
  'displayName', 'description', 'isReadOnly', 'isFilterDisplayed', 'isDissociateDisplayed',
  'widgetEdit', 'widgetDisplay', 'mappingValues', 'conditionalFormatting',
].join('|')

const ACTION_PROPS = ['position', 'isVisible', 'displayName', 'confirmation', 'segments', 'buttonType'].join('|')
...
const WHITELIST: Record<LayoutDomain, WhitelistEntry[]> = {
  folders: [
    ...
    entry(['replace'], `/folders/${NUM_OR_UUID}/children/${ID}/subChildren/${ID}/(position|isVisible)`),
  ],
  layout: [
    ...
    entry(['replace'], `/collections/${ID}/layout/segments/${ID}/(name|position|isVisible|filter|columns)`),
    entry(['replace'], `/collections/${ID}/layout/segments/${ID}/columns/${ID}/(position|isVisible)`),
    ...
    entry(['remove', 'replace'], `/collections/${ID}/layout/viewEdit/charts/${NUM_OR_UUID}/(${CHART_PROP_NAMES})`),
    entry(['replace'], `/collections/${ID}/layout/viewEdit/rows/${ID}/(position|isVisible|explorerConfiguration)`),
    ...
    entry(['replace'], `/collections/${ID}/layout/viewLists/${ID}/(name|position|recordsPerPage|allowJavascript)`),
    ...
    entry(['remove', 'replace'], `/dashboards/${NUM_OR_UUID}/charts/${NUM_OR_UUID}/(${CHART_PROP_NAMES})`),
    ...
    entry(['replace'], `/workspaces/${ID}/components/${NUM_OR_UUID}/(name|displaySettings|visibility|options)`),
    entry(['add'], `/inboxes/-`),
...
export function matchesWhitelist(domain: LayoutDomain, op: {op: string; path: string}): boolean {
  return WHITELIST[domain].some(
    candidate => candidate.ops.includes(op.op as WhitelistEntry['ops'][number]) && candidate.pattern.test(op.path),
  )
}
